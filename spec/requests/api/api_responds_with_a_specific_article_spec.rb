RSpec.describe 'GET /api/articles/:id', type: :request do
  ...
  let!(:article) { create(:article, title: 'Fun with RSpeck!', body: 'This is the content of my article...') }
  ...
  it 'is expected to include title' do
    expect(JSON.parse(response.body)['article']['title']).to eq 'Fun with RSpeck!'
  end
